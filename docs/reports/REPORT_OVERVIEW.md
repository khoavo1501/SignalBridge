...
- **Cập nhật gần nhất:** 2026-09-21 — **chốt hướng deploy: AWS free tier, EC2 t3.small** (§8 trong [HUONG-DAN-SETUP-VA-DEPLOY.md](HUONG-DAN-SETUP-VA-DEPLOY.md)); trước đó 2026-09-20: **chống lag chart slave + cửa sổ trượt realtime** ([PERF-chart-realtime.md](PERF-chart-realtime.md)) + **hướng dẫn setup & deploy**; hết 9/9 milestone phase 1 ([M8-admin-ui.md](M8-admin-ui.md))
...
| 2026-09-20 | **Chart slave: mọi cửa sổ dùng agg backend (15m/2s · 1h/5s · 6h/10s · 24h/1m) + lưới thời gian cố định trượt theo ô agg** — hết giật do raw hàng nghìn điểm; ô trước data đầu tiên = 0, gap giữa chừng vẫn null (giữ thiết kế hc0 M7); WS tail bucket latest-wins + `raw` tsMs riêng cho badge; sparkline seed `agg=2s` pad 0 đủ 90 → không "chạy lại" khi reload | PERF report §2/§3/§4, yêu cầu trực tiếp của user |
...
| 2026-09-20 | **Tài liệu vận hành** — [HUONG-DAN-SETUP-VA-DEPLOY.md](HUONG-DAN-SETUP-VA-DEPLOY.md): hướng dẫn setup/deploy từ clone repo (`.env` secrets, compose tự migrate+seed), bàn giao broker Q1 `192.168.1.3:1883`, đăng ký gateway qua `/admin` hoặc tự upsert từ `info`, dev mode (lưu ý esbuild postinstall, Python 3.14), vận hành hằng ngày + backup, checklist verify deploy, hardening production còn mở, bảng troubleshooting các sự cố đã gặp qua M0–M8 |
| 2026-09-20 | **PERF ✅ chart slave + cửa sổ trượt** (user report giật 1h + "chạy lại" khi reload): mọi cửa sổ agg backend (1h: raw ~5.000→720 điểm), lưới ô cố định trượt theo `anchor`, leading = 0, gap giữa chừng vẫn null, WS tail bucket + `raw` tsMs cho badge, sparkline pad 0 đủ 90, bỏ `setTail([])` trong refresh 15 s. Verify live: trôi trục đúng nhịp, 24h tự đệm 347 ô 0, 90/90 sparkline ngay sau reload. Bug bắt được khi verify: `nSlots` chia giây cho ms → 0 điểm. Test lại toàn dự án: 83 pytest, ruff/black, prettier/eslint/build, 7 service healthy, e2e WS 250 ms PASS, error contract 404/400, smoke 4 route 0 console error |
| 2026-09-21 | **Hướng deploy chốt lại: AWS free tier, EC2 `t3.small` (2 GB)** — phương án cloud cũ bất khả thi hết dung lượng tạo VM, bị loại khỏi dự án. Chính sách free mới: $100–200 credit / 6 tháng rồi auto-close; "Free tier eligible" = trừ credit. Q1 bàn giao firmware đổi thành **Elastic IP AWS :1883** (gateway ở xa, không cùng LAN). Thêm §8 vào HUONG-DAN-SETUP-VA-DEPLOY.md (swap, mem_limit, log rotation, SG 22/80/1883, EMQX ACL vì firmware không có credential); sửa §7: repo **đã có** remote `khoavo1501/SignalBridge` nhưng origin lùi 21 commit → **blocker: phải push trước khi clone trên VM** |
| 2026-09-21 | **CI/CD** — CI GitHub Actions xác nhận đang chạy xanh trên `main` (repo đã push hết, `origin/main`=`681d919`); thêm job `infra-validate` (`compose config -q`) + concurrency + workflow_dispatch vào `ci.yml`; tạo `deploy.yml` (CD): build ảnh backend/frontend trên runner → push GHCR theo `<sha>` → SSH `compose pull && up -d` + health gate, mặc định tắt qua biến repo `DEPLOY_ENABLED`, secrets `DEPLOY_*` (+`GHCR_USER/PAT` nếu package private); compose thêm `image: ${IMAGE_REPO:-signalbridge}-{backend,frontend}:${IMAGE_TAG:-latest}` (dev không đổi hành vi); docs §10 hướng dẫn bật CD + sửa 2 mục stale ("chưa push 21 commit") |
| 2026-09-21 | **Docs deploy walkthrough** — §8 HUONG-DAN viết lại thành 8.1–8.6 (Console từng bước → swap+daemon.json+docker → clone GitHub → `docker-compose.override.yml` mem_limit 7 service → up+checklist+Elastic IP firmware → chuyển sang CD §10); troubleshooting sửa dòng firmware (Elastic IP thay 192.168.1.3 cho production). **Fix `deploy.yml`: `DEPLOY_PATH`/`GHCR_USER`/`GHCR_PAT` có trong `envs` nhưng chưa map từ secrets → script `set -u` sẽ fail ở lần CD đầu; đã thêm 3 dòng `env:`** |
| 2026-09-26 | **Tài liệu vận hành server AWS** — [HUONG-DAN-VAN-HANH-SERVER-AWS.md](HUONG-DAN-VAN-HANH-SERVER-AWS.md): stop/start (giữ Elastic IP, tắt Stop protection trước khi stop, data không mất nhưng telemetry QoS 0 mất trong thời gian tắt), bảng chi phí theo trạng thái (stopped ~$5/tháng, quên Release EIP vẫn bị tính), SSH fail 5 bước chẩn đoán (IP VNPT đổi là số 1), việc định kỳ + mục "không làm" (`down -v`, đổi INFLUX_TOKEN sau init) |
